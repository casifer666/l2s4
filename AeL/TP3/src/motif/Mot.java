...
public class Mot {
  ...
  public int indiceMotifNaif(Mot motif) {
	int n = this.length(); int m = motif.length();
	for (int i = 0 ; i <= n - m ; i++) {
		if (this.m.substring(i,i+m).equals(motif.m)) {
			return i;
		}
	}
	return -1 ;
  ...
  public int indiceMotifAutomate(Mot motif) {
  	if (motif.length() == 0) return 0;
		...
		AutomateMotif autoMotif = new AutomateMotif(motif); int etat = autoMotif.etatInitial();
	 	for (int i = 0 ; i < this.length() ; i++) {
			etat = autoMotif.transition(etat,this.m.charAt(i));
			if (etat == autoMotif.etatFinal()) return (i + 1 - motif.length());
		}
	 	return -1 ;
  	}
  }
  ...
  public int indiceMotifKMP(Mot motif){
  	if (motif.length() == 0) return 0;
  		...
  		int[] pref = calculerFonctionPrefixe(motif); int q = 0;
  		for (int i = 0 ; i < this.length() ; i++) {
			...
			if (q == motif.length()) return (i-motif.length()+1);
		}
		return -1 ;
 	}
  }
  ...
  private int[] calculerFonctionPrefixe(Mot motif){
	int[] pref = new int[motif.length()]; int k = 0;
  ...
  /**
   * le programme principal permet de tester les methodes
  ...
  public static void main(String args[]){
	...
	System.out.println("indice motif naif : "+ mot.indiceMotifNaif(new Mot(args[1])));
  System.out.println("indice motif automate : "+ mot.indiceMotifNaif(new Mot(args[1])));
	System.out.println("indice motif KMP : "+ mot.indiceMotifKMP(new Mot(args[1]))) ;
